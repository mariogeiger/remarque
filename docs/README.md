# Remarque documentation

- [Architecture](architecture.md): component boundaries and dependency direction.
- [Paper Pro field guide](paper-pro-field-guide.md): firmware and device facts.
- [Remote access](remote-access.md): tablet connectivity and service setup.
- [Application](../app/README.md): product crates, deployment, and build adapters.
- [Native reverse engineering](../reverse-engineering/README.md): observation tools and evidence.
- [Xochitl findings](../reverse-engineering/xochitl.md): recovered native behavior.
- [Core](../app/core/README.md): portable behavioral contracts.
- [Native replay](../reverse-engineering/native-replay/README.md): differential validation.
